...
class OTPAuthViewModel extends ChangeNotifier
{
  ...
  void sendOTPForVerification({required bool isForgetPass, required String email, String? pass,required BuildContext context,}) async {
    ...
    else
    {
      ...
      LoginRepository().forgetPasswordVerifyOTPLoginAPI({"email": email, "otp": otp}).then((otpRes){
            debugPrint("OTP Matching result received");
            if(otpRes == null || otpRes['statusCode'] == null || otpRes['statusCode'].toString().isEmpty || otpRes['statusCode'].toString() == "null"){
            ...
            tokenReset = token;
            showBottomSheetForPasswordReset(token: token, email: email, context: context);
  ...
  resendOTP(String email) async {
    ...
    // sec = 60;
    // startTimer();
    await LoginRepository().resendPassword({'email':email}).then((value){
  ...
  showBottomSheetForPasswordReset({required String email,required String token,required BuildContext context}){
        ...
        child: Column(
          children: [
            ...
            AppRoundedButton(onTap: (){
              apiCall(context);
            }, title: "Reset", isEnable: true, buttonColor: AppColors.blueSplashScreen, textColor: AppColors.white,),
  ...
  Future apiCall(BuildContext context) async {
    ...
    if(passResetCont.text.toString().trim() != confPassResetCont.text.toString().trim()){
      CustomToast(context: context, message: "Password in both field must be same");
      return;
    }
    LoginRepository().resetPasswordForgetPasswordLoginAPI(token: tokenReset, pass: passResetCont.text.toString().trim()).then((value){
